...
pub async fn get_anchor_metrics_with_rpc(
    ...
    let result: AnchorMetrics = circuit_breaker
        .call(|| async {
            with_retry(
                || async {
                    rpc_client
        ...
        .map_err(|e| match e {
            ...
            failsafe::Error::Inner(err) => anyhow::anyhow!(err.to_string()),
        })?;
