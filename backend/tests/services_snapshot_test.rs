...
fn test_same_content_same_hash() {
    let now = Utc::now();
    let hash1 = SnapshotService::hash_snapshot_hex(AnalyticsSnapshot::new(42, now)).unwrap();
    let hash2 = SnapshotService::hash_snapshot_hex(AnalyticsSnapshot::new(42, now)).unwrap();
...
fn test_different_epoch_different_hash() {
    let now = Utc::now();
    let hash1 = SnapshotService::hash_snapshot_hex(AnalyticsSnapshot::new(1, now)).unwrap();
    let hash2 = SnapshotService::hash_snapshot_hex(AnalyticsSnapshot::new(2, now)).unwrap();
